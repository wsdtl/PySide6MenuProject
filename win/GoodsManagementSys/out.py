...
class OutTableGoods(QTableWidget):
    def __init__(
        self,
        parent: "QWidget",
        FindSingal: "Signal",
        ...
        self.setObjectName("OutTableGoods")
        self.setStyleSheet(
            """
            QTableWidget#TableGoods{
                outline: none;
            }
            QTableWidget#TableGoods::item:selected
            {
    ...
    def OnFindSingal(self, msg: str):
        if msg:
            query = select(ALL_TABLE["goods_base"]).filter(
                ALL_TABLE["goods_base"].c["货物名称"].regexp_match(msg)
            )
        else:
            query = select(ALL_TABLE["goods_base"])
    ...
    def choose(self):
        ...
        self.ChooseSingal.emit(
            {
                "货物编号": self.item(index, 0).text(),
                "货物名称": self.item(index, 1).text(),
                "规格": self.item(index, 2).text(),
                "类别": self.item(index, 4).text(),
                "单位": self.item(index, 5).text(),
                "卖方": self.item(index, 6).text(),
                "上限": self.item(index, 3).text(),
            },
        )


class OutTableChoose(QTableWidget):
    ...
    def __init__(
        ...
        # 隔行变色
        self.setAlternatingRowColors(True)
        self.setObjectName("OutTableChoose")
        self.setStyleSheet(
            """
            QTableWidget#OutTableChoose{
                outline: none;
            }
            QTableWidget#OutTableChoose::item:selected
            {
                background-color: #1ecc94; 
            }
        """
    ...
    def choose(self):
        # 获取当前行 Index
        index = self.currentIndex().row()
        goods_id = self.item(index, 6).text()
        self.goods_dict[goods_id]["数量"] -= 1
        if self.goods_dict[goods_id]["数量"] <= 0:
            self.goods_dict.pop(goods_id)
        self.updata_table()

    def OnChooseSingal(self, msg: dict):

        if msg["货物编号"] in self.goods_dict:
            if self.goods_dict[msg["货物编号"]]["数量"] < int(msg["上限"]):
                self.goods_dict[msg["货物编号"]]["数量"] += 1
                self.updata_table()
            else:
...
class OutTableQwidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.but_out = QPushButton(text="出库")
        self.but_out.setFixedWidth(100)
        ...
        self.but_out.clicked.connect(lambda: self.OutSingal.emit())
        # 索引变化时触发
