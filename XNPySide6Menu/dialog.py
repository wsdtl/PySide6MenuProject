...
from PySide6.QtGui import QPixmap, QFont, QPainter, QPaintEvent, QColor, QFontMetrics
...
class DialogOver(QWidget):
    """DialogOver初始化对象 DialogOver下文叫弹窗

    参数:
        parent (MainWindow): 主MainWindow窗口, 弹窗判断位置用\n
        text (str): 弹窗内容\n
        title (Optional[str], optional): 弹窗标题. Defaults to "".\n
        flags (Optional[str], optional): 弹窗类型. Defaults to "warning"or"danger"or"success".\n
        _showTime (Optional[int], optional): 展示时间. Defaults to 1500.\n
        _dieTime (Optional[int], optional): 消亡时间. Defaults to 500.
    """

    _instanceWidget: queue.Queue = queue.Queue(
        7
    )  # 存储实例化对象序号的队列，供paintEvent调用时判断位置
    _instanceDel: queue.Queue = queue.Queue(
        7
    )  # 存储实例化对象序号的队列, 供__del__调用时判断位置
    ...
    def __del__(self) -> None:
        """将被删除对象的位置在_count中置为0, 标记为空闲位置"""
        DialogOver._count[DialogOver._instanceDel.get()] = (
            0  # 将被删除对象的位置在_count中置为0，标记为空闲位置
        )
    ...
    def __init__(
        ...
        self.moveSize = 15  # 窗口的初始高度偏移量, 第一个窗口与父对象窗口上限的间隙
        ...
        self.dieTime.timeout.connect(
            self.closeDialog
        )  # 关闭时间到达后调用closeDialog方法
        self.show()
    ...
    def drawDialog(self, event: QPaintEvent, painter: QPainter, flags: str) -> None:
        ...
        titleFont = QFont("Microsoft YaHei UI", 12, QFont.Bold)  # 标题字体
        textFont = QFont("Microsoft YaHei UI", 12)  # 文字字体
        icoSize = 40  # 图标大小
        titleWidth = (
            QFontMetrics(titleFont).horizontalAdvance(self._title) + 10
        )  # 图标宽度, 间隙 10px
        ...
        painter.drawRoundedRect(
            backgroundX, backgroundY, backgroundWidth, backgroundHight, 20.0, 20.0
        )
        # 画阴影
        ...
        painter.drawPixmap(
            ...
            30,
            30,
            QPixmap(f":/image/{flags}.png").scaled(
        ...
        titleSizeMoveH = (
            backgroundHight - QFontMetrics(titleFont).height()
        ) // 2  # 计算标题上方间隙距离
        painter.drawText(
            backgroundX + icoSize,
            titleSizeMoveH,
    ...
    def setColor(self, flags: str) -> Tuple[QColor, QColor, QColor]:
        ...
        返回:
            Tuple[QColor, QColor, QColor]: [背景颜色, 边框颜色, 文字颜色]
        """
        if flags == "success":
    ...
    def moveDialog(self) -> None:
        ...
        y = (
            self._parent.y()
            + self.moveSize
            + (DialogOver._instanceWidget.get() * (self.height() + self.moveSizeH))
        )
